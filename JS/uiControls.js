...
sortedEventTypes.forEach(eventType => {
    ...
    // --- ADD THIS NEW BLOCK ---
    // Add dynamic loading trigger for specific types
    if (eventType === 'Minuscule' || eventType === 'Lectionary') {
        checkbox.addEventListener('change', () => {
            // Only trigger load when the box is CHECKED
            if (checkbox.checked) {
                // Call the new global function from load-more.js
                // Pass the eventType and the label element for UX feedback
                if(typeof triggerDynamicLoad === 'function') {
                    triggerDynamicLoad(eventType, label);
                } else {
                    console.error('triggerDynamicLoad function not found!');
                }
            }
        });
    }
